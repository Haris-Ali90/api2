...
class ChatController extends Controller
{
    ...
    public function createThreads(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            if (in_array($request->source,[1,2,3]))
            {
                ...
                if($request->user_type != Thread::Joey_Type)
                {
                    $thread_id=Thread::
                    ...
                    if($thread_id==null)
                    {
                        $thread_id=Thread::where('threads.other_user_id','=',$request->get('user_id'))
                    ...
                    if($thread_id==null)
                    {
                        ...
                        $thread_id->is_accepted=0;
                        $thread_id->is_thread_end = 0;
                        $thread_id->thread_reason_id=$request->get('thread_reason_id');
                        $thread_id->save();
                        $path = public_path().'/public/thread/' . $thread_id->id;

                        File::makeDirectory($path, $mode = 0777, true, true);
                    }
                }
                elseif($request->user_type == Thread::Joey_Type)
                {
            ...
            elseif ($request->source == 4)
            {
                $this->validate($request,
                    [
                        'email' => 'required|email|max:190',
                    ]);
                $joeyco_user = JoeycoUsers::where('email_address',$request->email)->first();

                if (isset($joeyco_user))
                {

                    $thread_id = Thread::where('threads.user_id', '=', $joeyco_user->id)
                        ->where('threads.other_user_id', '=', $joeyco_user->id)
                        ->where('threads.creator_type', '=', $joeyco_user->type)
                        ->where('threads.other_user_type', '=', $request->other_user_type)
                        ->where('threads.is_thread_end', '=', 0)
                        ->first();
                    if ($thread_id == null) {
                        $thread_id = Thread::where('threads.user_id', '=', $joeyco_user->id)
                    ...
                    if ($thread_id == null) {
                        $thread_id = new Thread();
                        $thread_id->user_id = $joeyco_user->id;
                        $thread_id->creator_type = $joeyco_user->type;
                        $thread_id->other_user_id = $joeyco_user->id;
                        $thread_id->other_user_type = $request->other_user_type;
                        $thread_id->is_accepted = 0;
                        $thread_id->is_thread_end = 0;
                        $thread_id->thread_reason_id = $request->get('thread_reason_id');
                        $thread_id->save();
                ...
                else
                {
                    ...
                    $joeyco_user=new JoeycoUsers();
                    $joeyco_user->email_address =$request->get('email');
                    $joeyco_user->full_name = $request->get('full_name');
                    //$joeyco_user->full_name = $username=strstr($joeyco_user->email_address,'@',true).$randomid;
                   $joeyco_user->type= Thread::Guest_Type;
                    $joeyco_user->save();
        ...
        try {
            if (in_array($request->source,[1,2,3])) {
                ...
                if (($request->get('thread_id') != null)) {
                    ...
                    $thread_id = Thread::where('threads.user_id', '=', $request->get('user_id'))
                        ->where('threads.other_user_id', '=', $request->get('other_user_id'))
                        ->where('threads.creator_type', '=', $request->get('user_type'))
                        ->where('threads.other_user_type', '=', $request->get('other_user_type'))
                        ->where('threads.is_thread_end', '=', 0)
                        ->first();
                    if ($thread_id == null) {
                        $thread_id = Thread::where('threads.user_id', '=', $request->get('other_user_id'))
                            ->where('threads.other_user_id', '=', $request->get('user_id'))
                            ->where('threads.creator_type', '=', $request->get('other_user_type'))
                            ->where('threads.other_user_type', '=', $request->get('user_type'))
                            ->where('threads.is_thread_end', '=', 0)
            ...
            elseif($request->source == 4)
            {
                $guestUser = JoeycoUsers::where('email_address',$request->email)->first();
                $response = [];
                if ($request->get('thread_id') != null) {
                    $this->validate($request,
                        [
                ...
                /*$guestUser=new JoeycoUsers();
                $guestUser->email_address=$request->get('guest_email');
                $guestUser->type= Thread::Guest_Type;
                $guestUser->save();*/
                //if (($request->get('thread_id') != null)) {
                //    $thread_id = Thread::where('id', '=', $request->get('thread_id'))->first();

                //} else {
                    $thread_id = Thread::where('threads.user_id', '=', $guestUser->id)
                        ->where('threads.creator_type', '=', $guestUser->type)
                        ->where('threads.is_thread_end', '=', 0)
                        ->first();
                    if($thread_id === null){
                        return RestAPI::response('No data found', false, 'error_exception');
                    ...
                    Message::where('thread_id', $thread_id->id)
                        // ->where('creator_type', '!=', $request->get('user_type'))
                        ->where('is_read', '=', 0)
                        ->update(['is_read' => 1]);
                //}
            }
    ...
    public function addParticipants(Request $request)
    {
        ...
        try {
            $thread=Thread::where('threads.is_thread_end','=',0)
    ...
    public function sendMessage(Request $request)
    {
        ...
        try {
            ...
            if(preg_match('/^http\:\/\/|https\:\/\/|www\.google$/',$request->get('message')))
            {
                $message->message_type='GMAP';
            }
            else
            {
            ...
            if($request->hasfile('files'))
            {
                ...
                foreach($request->file('files') as $file)
                {
                    ...
                    $path_org = "public/thread/".$thread->id."/";
                    ...
                    $filepath="public/thread/".$thread->id."/".$name;
    ...
    public function threadList(Request $request)
    {
        ...
        try {
            $threads=Thread::where('threads.other_user_type','=',$request->user_type)
                ->where('threads.is_thread_end','=',0)
                ->whereNull('threads.deleted_at')
                ->where('is_accepted','=',0)
                ->get();
    ...
    public function activeThreadList(Request $request)
    {
        ...
        try {
            $threads=Thread::where('threads.creator_type','=',$request->user_type)
                ->where('threads.user_id','=',$request->user_id)
                ->where('threads.is_thread_end','=',0)
                ->whereNull('threads.deleted_at')
                ->where('is_accepted','=',1)
                ->get();
        ...
        try {
            $threads=Thread::withCount(['chatUnseenMessage' => function ($query) use ($request){
                $query->where('sender_id','!=',$request->joey_id)
                    ->where('creator_type','!=',Thread::Joey_Type);
            }])->where('threads.creator_type','=',Thread::Joey_Type)
...
//                'user_id'=>'required|exists:onboarding_users,id'
        ...
        DB::beginTransaction();
        try {
            $threadUser         =   Thread::where('threads.creator_type','=',$request->user_type)->where('user_id',$request->user_id)->pluck('id')->toArray();
            $otherThreadUser    =   Thread::where('threads.other_user_type','=',$request->user_type)->where('other_user_id',$request->user_id)->pluck('id')->toArray();
        ...
        return RestAPI::response($response, true, "User List ");
    }
    ...
    public function messageGroupsCreate(Request $request)
    {
        ...
        try {
            ...
            else
            {
                ...
                $messageGroup->save();
                $path = public_path().'/public/group/' . $messageGroup->id;
                File::makeDirectory($path, $mode = 0777, true, true);
                if($request->hasfile('message_group_pic'))
                {
                    ...
                    $filepath="public/group/".$messageGroup->id."/".$name;
    ...
    public function addGroupMember(Request $request)
    {
        ...
        try {
            ...
            $user = ModelFindByType($request->user_type,$request->email);


            if($user==null)
            {
                return RestAPI::response('Email Does not exist.', false, 'error_exception');
            }
            ...
            $group = MessageGroups::where('id',$request->group_id)->first();
            $response = [
        ...
        try {
            ...
            if($request->hasfile('files'))
            {
                ...
                foreach($request->file('files') as $file)
                {
                    ...
                    $message_file =new MessageFile();
                    $path_org ='public/group/' . $message->group_id;
                    $name=$file->getClientOriginalName();
                    $extension = pathinfo($name, PATHINFO_EXTENSION);
                    ...
                    $name=$rand_num.'-opimg-'.$name;
                    $file->move($path_org,$name);
                    $filepath='public/group/' . $message->group_id."/".$name;
                    $message_file->file_name=$filepath;
                    if(in_array($extension,["pdf","xlx","csv"]))
                    {
                        $message_file->file_type="PDF";
...
//            }
            ...
            {
                $groupMembers= $groupMembers->where('user_id','!=',$request->user_id)->where('creator_type','!=',$request->user_type);
    ...
    public function userAllgroupDetail(Request $request)
    {
        ...
        try {
            ...
            $messageGroups=MessageGroups::withCount('getGroupMember')->
    ...
    public function allChatUserResource($threadUserData,$user_id)
    {
        ...
        foreach($threadUserData as $threadUser)
        {
            ...
            {
                if($threadUser->other_user_type== Thread::Onboarding_Type)
                {

                    $sender=Onboarding::find($threadUser->user_id);

                }
                elseif($threadUser->other_user_type==Thread::Joey_Type)
                {

                    $sender=Joey::find($threadUser->user_id);

                }
                elseif($threadUser->other_user_type==Thread::Dashboard_Type || $threadUser->other_user_type=="Routing") {

                    $sender = Dashboard::find($threadUser->user_id);

                }
                elseif ($threadUser->other_user_type==Thread::Merchant_Type)
                {
                    $sender = JoeycoUsers::find($threadUser->user_id);
                }
                elseif ($threadUser->other_user_type==Thread::Guest_Type)
                {
                    $sender = JoeycoUsers::find($threadUser->user_id);
                }

                $response[$threadUser->user_id]= [
                    'user'=> isset($sender) ? new SenderResource($sender) : null,
